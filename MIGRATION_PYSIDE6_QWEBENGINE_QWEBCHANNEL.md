...
- ? Step 7 ?? Qt Rest overlay ?????????????overlay ?????/????????
- ???????? Step 7 ???????????????????????????????? Step 11 ??????????????
...
- ??? Qt smoke ???? notify ???`created -> html_loaded -> channel_ready -> show -> autoHide/dismiss -> hide`
- ??? `debug_trigger_notify_show()` ????? `NotificationManager -> dispatcher -> Qt notify host` ???? Qt notify?????? probe
- ?????????????? `debug_force_rest_due()` ? controller ?? `should_prompt=True` ???? `NotificationManager.on_snapshot(...)`?Qt notify ?????
- ???????`notify_ready_for_show` ???????Step 8 ?????? `rest / snooze / dismiss` ???????????????? dismiss
...
- ????????
- HTML/CSS/JS ??????
- ? `category_names / categories/delete / update_check / open_url` ?????? API ?????

### ????

- ?????? Qt bridge data URL ???????????????????????
- ?????? Rest ??????????Qt ??????? `Audio('/assets/...')`?
- ???????????? `file:///...` ?????????????? HTTP ??????????????????
- Rest ????????? `file:///...` ??? overlay ????????? HTTP ??????????????????
- Notify ????????????? `file:///...` ????????????????????????????
- ?????GitHub releases API ?? `404` ??????? `ok=true / has_update=false`??????????
...
- ???? T9-1?????????????`qt.icon_probe` ??????
- ???? T9-2?Rest ??????????`qt.rest_end_sound_probe` ??????
- ???? T9-3?Qt smoke ????? `qt.main_static_probe / qt.rest_static_probe / qt.notify_static_probe`???????????????
- ???? T9-4?`qt.update_check_probe` ???? 404 ??????

---

### ?????2026-04-16?

- `qt.main_static_probe` ????????????? `http://127.0.0.1:17993/assets/...`?
- `qt.rest_static_probe` ??? Rest overlay ??????? `http://127.0.0.1:17993/rest/...` ? `http://127.0.0.1:17993/assets/...`?
- `qt.notify_static_probe` ???? Notify ?????? `file:///...` ????????????
- `qt.icon_probe` ???????? bridge ??????
- `qt.rest_end_sound_probe` ??????Rest ??????????
- `qt.update_check_probe={"ok":true,"has_update":false,"error":""}`??? 404 ?????????
...
- ???? T10-1????? `main.py` ??? Qt ?????????????????
- ???? T10-2??????????????? Qt smoke?????Rest?Notify ?????
- ???? T10-3????? `main.py --host legacy` ?????????????????? `legacy_pywebview`?
...
- `main.py` ?????? `legacy` ?? `qt`?
- ???? `python main.py --no-single` ??????? `host=qt startup`?? Qt ????Rest?Notify smoke ?????
- ???? `python main.py --host legacy --no-single` ??????? `host=legacy_pywebview startup`????????????
- Step 10 ?????? `T10-1` ???`T10-3` ???
- Step 10 ?????????????????? Qt ??????????????????????????????????????????????????
...
- ?????2026-04-16??
  - `QSystemTrayIcon + QMenu` ?????? Qt ?????????????????? `pystray`?
  - ?????????????????????????????????????????????????
  - `??????` ????????????????????????????????????? smoke?
  - `T10A-1` ????? 20 ??????????????????????????????
